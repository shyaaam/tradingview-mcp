    ...
    _deps: {
      ...
      evaluateBound: async (expression) => {
        ...
        return {
          ...
          plots: [
            { plot_name: 'TVOBS_HTF_V1_PVP_RAIL', raw_value: '24123.50' },
            { plot_name: 'TVOBS_LTF_V1_DIVERGENCE', raw_value: 'bullish' },
          ],
        };
      },
...
test('named plot capture fails closed on missing, duplicate, or stale readback', async () => {
  ...
  for (const result of [
    ...
    { pane_index: 3, pane_count: 8, symbol: 'BITSTAMP:BTCUSDT', timeframe: '60', study_id: input.study_id, study_name: input.study_name, plots: [{ plot_name: input.plot_names[0], raw_value: '1' }] },
    { pane_index: 3, pane_count: 8, symbol: 'BITSTAMP:ETHUSDT', timeframe: '60', study_id: input.study_id, study_name: input.study_name, plots: [{ plot_name: input.plot_names[0], raw_value: '1' }, { plot_name: input.plot_names[1], raw_value: '2' }] },
  ]) {
    await assert.rejects(
