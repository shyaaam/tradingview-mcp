...
export async function list() {
  const result = await evaluate(`
    (function() {
      ...
      var all = cwc.getAll();
      // TradingView can retain hidden chart widgets after a layout downgrade.
      // The inline chart count is the authoritative visible pane boundary.
      ...
      for (var j = 0; j < visible.length; j++) {
        try {
          if (visible[j].model && activeChart._chartWidget && visible[j] === activeChart._chartWidget) { activeIndex = j; break; }
        } catch(e) {}
      }
