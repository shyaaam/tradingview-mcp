...
export const observerToolDefinitions = Object.freeze({
  ...
  tv_observer_capture_pane_telemetry_ohlcv: {
    ...
    outputSchema: {
      ...
      study_telemetry_reason: z.literal('missing-or-ambiguous').nullable(),
      studies: z.array(z.object({
        study_id: z.string().min(1),
        study_name: z.string().min(1),
        values: z.array(z.object({
  ...
  tv_observer_capture_named_plot_telemetry: {
    ...
    inputSchema: {
      ...
      study_id: z.string().min(1),
      study_name: z.string().min(1),
      plot_names: z.array(z.string().regex(/^TVOBS_[A-Z0-9]+_V1_[A-Z0-9_]+$/)).min(1).max(64),
    },
    outputSchema: {
      success: z.literal(true),
      extraction_version: z.literal('observer-named-plot-telemetry-v1'),
      profile_id: z.string().min(1),
      chart_target_id: z.string().min(1),
      chart_id: z.string().min(1),
      layout_id: z.string().min(1),
