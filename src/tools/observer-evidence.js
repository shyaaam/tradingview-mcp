...
export function registerObserverEvidenceTools(server) {
  registerObserverTool(
    ...
    'Return exact identity evidence for the prepared observer session without exposing raw account identity.',
    async ({ ...args }) => {
  ...
  registerObserverTool(
    ...
    'Capture exactly one requested OHLCV candle from the exact prepared chart binding.',
    async ({ ...args }) => {
